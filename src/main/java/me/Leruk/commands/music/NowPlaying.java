...
public class NowPlaying extends ListenerAdapter {
    ...
    public void onMessageReceived(@NotNull MessageReceivedEvent e) {
        ...
        if (args[0].equalsIgnoreCase(DiscordBot.getPrefix() + "nowplaying")) {
            ...
            if (args.length > 1) {
                error.setDescription('`' + DiscordBot.getPrefix() + "nowplaying` - команда для отображения названия трека");
            ...
            {
                error.setDescription("Ничего не играет");

                tChannel.sendMessage(" ").setEmbeds(error.build()).queue();
                return;
            }

            tChannel.sendMessage("Сейчас играет: `")
